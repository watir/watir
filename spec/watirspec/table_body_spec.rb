...
describe "TableBody" do
  ...
  describe "#exists?" do
    ...
    it "returns true if the element exists (default how = :id)" do
      ...
      browser.table(:index, 0).body("first").should exist
    end
    ...
    it "raises TypeError when 'what' argument is invalid" do
      ...
      lambda { browser.table(:index, 0).body(:id, 3.14).exists? }.should raise_error(TypeError)
    end
    ...
    it "raises MissingWayOfFindingObjectException when 'how' argument is invalid" do
      ...
      lambda { browser.table(:index, 0).body(:no_such_how, 'some_value').exists? }.should raise_error(MissingWayOfFindingObjectException)
    end
  end
  ...
  describe "#[]" do
    bug "WTR-357", :watir do
      it "returns the row at the given index (page context)" do
        browser.body(:id, 'first')[0].text.should == 'March 2008'
        browser.body(:id, 'first')[1][0].text.should == 'Gregory House'
        browser.body(:id, 'first')[2][0].text.should == 'Hugh Laurie'
      end
    end

    it "returns the row at the given index (table context)" do
      browser.table(:index, 0).body(:id, 'first')[0].text.should == 'March 2008'
      browser.table(:index, 0).body(:id, 'first')[1][0].text.should == 'Gregory House'
      browser.table(:index, 0).body(:id, 'first')[2][0].text.should == 'Hugh Laurie'
    end
  end
