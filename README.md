...
If you are using [timecop](https://github.com/travisjeffery/timecop) (or any other time-freezing library), you should tell Watir to use `Timeout` instead of `Time` since latter is stubbed. This can be done by calling `Watir::Wait.use_timeout = true`.
