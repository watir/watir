        ...
        class RegexpDisassembler
          ...
          def substrings
            @substrings ||= begin
              ...
              while source.gsub!(/\[(?:[^\[\]]+)\]/, '.')
              end # replace character classes with wildcard
              source.gsub!(/\(\?<?[=!][^)]*\)/, '') # remove lookahead/lookbehind assertions
              # replace named, atomic, and non-matching groups with unnamed matching groups
              source.gsub!(/\(\?(?:<[^>]+>|>|:)/, '(')

              while source.gsub!(GROUP_REGEX) { |_m| simplify_group(Regexp.last_match) }
              end
              ...
              source.gsub!(/(?<char>.)#{COUNTED_REP_REGEX.source}/) do |_m| # repeat counted characters
                (Regexp.last_match[:char] * Regexp.last_match[:min_rep].to_i).tap do |str|
                  str << '.' if Regexp.last_match[:max_rep]
                end
              end
